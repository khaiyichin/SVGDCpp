...
Here detailed usage instructions is provided, but for specific API please consult the documentation (built with `-D BUILD_DOCUMENTATION=TRUE`).
...
### Basic usage
This example uses the provided classes `MultivariateNormal`, `GaussianRBF`, and `Adam` optimizer to estimate a mulitvariate normal distribution.
```cpp
...
### How to create a custom kernel or model functions?
Most likely, you'll want estimate your own density functions and/or use other types of kernel functions. There are in general 3 methods to provide them using this library, of which the first 2 are preferable over the third.
...
>:info: `VectorXADd` and `MatrixXADd` are aliases for `Eigen::VectorXd<CppAD::AD<double>>` and `Eigen::MatrixXd<CppAD::AD<double>>` respectively. The `CppAD::AD<double>` type is a `double` type that is used by the CppAD library to perform automatic differentiation. To convert between the `double` and `CppAD::AD<double>` types you can use `ConvertToCppAD` and `ConvertFromCppAD`.
...
In this method, you instantiate a `Model`/`Kernel` class and update the `Model`/`Kernel` functions by calling `UpdateModel`/`UpdateKernel` with your desired function object as the argument. The example below also show how you can compose new models from existing ones.
```cpp
...
If you wish to also run SVGD in [parallel mode](#multithreaded-execution), ensure that the `Clone*` functions are provided for polymorphic copy which is required for multithreaded execution of SVGD. Otherwise they are not necessary.
```cpp
...
Running SVGD in parallel mode is as simple as calling `SetupForParallelMode()` at the beginnning of your code and then instantiating the `SVGD` class with the `parallel` argument as `true`.
```cpp
...
If activated, it will run with the maximum number of threads the machine has. _Note: you're unlikely to see performance improvements if you only have a few particles ($n \leq 10$)._